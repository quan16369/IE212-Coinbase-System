...
kubectl -n feature-platform exec deploy/feature-platform -c feature-platform -- \
  python -c "import urllib.request; print(urllib.request.urlopen('http://localhost:8080/features/idempotency', timeout=10).read().decode())"

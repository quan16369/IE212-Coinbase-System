...
# Install Python packages and download JARs in single layer
RUN pip install --no-cache-dir -r requirements_paimon.txt && \
    wget -P /opt/spark/jars/ https://repo1.maven.org/maven2/org/apache/paimon/paimon-spark-3.3/0.6.0/paimon-spark-3.3-0.6.0.jar && \
    wget -P /opt/spark/jars/ https://repo1.maven.org/maven2/org/apache/hadoop/hadoop-aws/3.3.2/hadoop-aws-3.3.2.jar && \

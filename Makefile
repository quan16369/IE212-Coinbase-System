...
gke-workday-start:
	DEPLOY_SERVICES="$(DEPLOY_SERVICES)" INSTALL_OBSERVABILITY="$(INSTALL_OBSERVABILITY)" ENABLE_PUBLIC_INGRESS="$(ENABLE_PUBLIC_INGRESS)" bash scripts/start_gke_workday.sh
...
streaming-platform-build:
	docker build -f services/telemetry_producer/Dockerfile -t coinbase-kafka-producer:latest .
